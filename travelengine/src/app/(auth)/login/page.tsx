...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store in localStorage for client state reference if needed
    ...
    <Card className="border-slate-200/60 shadow-xl bg-white/80 backdrop-blur-md rounded-3xl overflow-hidden hover:shadow-2xl transition-shadow duration-300">
      <CardHeader className="space-y-2 text-center pb-6">
        ...
        <CardTitle className="text-2xl font-bold tracking-tight text-slate-800">
          Sign In to TravelEngine
        </CardTitle>
        <CardDescription className="text-slate-500 text-sm">
          Enter your email below to access your travel dashboard
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <Button
          ...
          disabled={loading}
          onClick={() => {
            toast.info("Google OAuth is simulated for this hackathon demo. Simply sign in with an email & password.");
          }}
        >
